...
import { cuetBestForParams } from "@/lib/static-params";
  ...
  const matches = colleges
    .filter(
      (c) =>
        ...
        c.coursesViaCuet.some((x) => slugify(x) === params.course),
    )
    .sort((a, b) => (b.lastGenCutoff ?? 0) - (a.lastGenCutoff ?? 0));
  if (!matches.length) throw notFound();
  const course = matches[0].coursesViaCuet.find(
    (x) => slugify(x) === params.course,
  )!;
...
export default async function BestFor({ params }: { params: Promise<Params> }) {
  const { course, matches } = load(await params);
  return (
    <ContentPage
      ...
      intro={
        <>
          The colleges below admit for {course} via CUET, ranked by their latest
          cutoffs. Use the order to plan your CSAS preferences for {course}.
        </>
      }
