...
import { cuetCollegeParams } from "@/lib/static-params";
...
import {
  colleges,
  isCollegePublishable,
  type CollegeRow,
} from "@/data/colleges";

// Programmatic college profile (SOP H7). Emits ONLY for publishable rows.
type Params = { university: string; college: string };

export function generateStaticParams(): Params[] {
  return cuetCollegeParams();
}
...
function buildMetadata(params: Params): Metadata {
  const c = colleges.find(
    (x) =>
      x.universitySlug === params.university &&
      x.collegeSlug === params.college,
  );
  return pageHead({
...
export default async function CollegeProfile({
  ...
  const { college } = load(await params);
  return (
    <ContentPage
      ...
      intro={
        <>
          {college.college} ({college.university}) admits via CUET for{" "}
          {college.coursesViaCuet.join(", ")}. Below are its required subjects,
          latest cutoff and seats — verified from official sources.
        </>
      }
    ...
    >
      <Section id="profile" heading={`${college.college} — CUET profile`}>
        <ul className="list-disc space-y-2 pl-5">
          <li>
            <strong className="text-ink">Courses via CUET:</strong>{" "}
            {college.coursesViaCuet.join(", ")}
          </li>
          <li>
            <strong className="text-ink">Required subjects:</strong>{" "}
            {college.requiredSubjects.join(", ")}
          </li>
          <li>
            <strong className="text-ink">Last General cutoff:</strong>{" "}
            {college.lastGenCutoff}
          </li>
          <li>
            <strong className="text-ink">Seats:</strong> {college.seats}
          </li>
        </ul>
        <p className="text-sm">
          Provider: {BRAND} — predict your fit with the{" "}
          <a
