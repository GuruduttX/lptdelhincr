...
export default function CutoffDataset() {
  ...
  return (
    <ContentPage
      ...
      intro={
        <>
          A structured, multi-year CUET cutoff dataset covering college × course
          × category × round, sourced from official university CSAS releases.
          Use the table below or download the CSV. Every figure is dated and
          sourced; unverified cells are clearly marked.
        </>
      }
      toc={[
      ...
      schema={[
        datasetSchema({
          ...
          description:
            "Multi-year CUET cutoffs sourced from official university CSAS releases. Columns: year, university, college, course, category, round, cutoff, source_url, verified_date.",
    ...
    >
      <Section id="download" heading="Download the dataset">
        <p>
          <strong className="text-ink">Free CSV.</strong> Machine-readable, one
          row per college × course × category × round.
        </p>
        <a
          href="/cuet/cutoff/dataset.csv"
          download
          className="btn-primary inline-flex w-fit"
        >
          <Download className="h-4 w-4" strokeWidth={2} /> Download CUET cutoff
          dataset (CSV)
        </a>
        {illustrative && (
          <p className="text-sm text-brand">
            Note: the dataset currently contains illustrative placeholder rows.
            Verified CSAS figures will replace them before publishing.
          </p>
        )}
      </Section>
      ...
      <Section id="notes" heading="Sources & method">
        <p>
          Each row is sourced from the relevant official CSAS release and
          carries a <code>source_url</code> and <code>verified_date</code>. We
          publish a cutoff only after it is verified against the official source
          — see{" "}
          <a
