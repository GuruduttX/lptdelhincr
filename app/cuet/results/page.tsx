...
export default function ResultsHub() {
  return (
    <HubPage
      ...
      intro={
        <>
          After CUET, the NTA releases your normalised score and percentile;
          universities then use it for admission. Explore the tools and guides
          below — all clearly labelled as estimates where they depend on the
      ...
      schema={[
        itemListSchema(
          getSiblings("/cuet/results").map((t) => ({
            name: t.label,
            url: t.href,
          })),
        ),
    ...
    >
      <p>
        <strong className="text-ink">Note:</strong> percentile, normalisation
        and predictor outputs are estimates that depend on the session's
        distribution — always confirm exact figures on your official scorecard.
      </p>
    </HubPage>
  );
}
