...
export default async function CompositeScore({
  params,
}: {
  params: Promise<Params>;
}) {
  ...
  const faqs: FAQItem[] = [
    {
      ...
      a: (
        <>
          {institute.name} combines your IPMAT aptitude score with the Personal
          Interview and academic/diversity factors into a composite. The exact
          weights vary by cycle and are set by the institute — verify them on
          the official admission policy before relying on any split.
        </>
      ),
    },
    {
      ...
      a: (
        <>
          No. A high aptitude score earns the interview call, but the final
          offer weighs the full composite, including the Personal Interview.
          Prepare for the PI as seriously as the written test.
        </>
      ),
    },
  ];
  ...
  return (
    <ContentPage
      ...
      intro={
        <>
          {institute.name} selects on a <strong>composite score</strong>, not
          the IPMAT result alone: your aptitude score is combined with the
          Personal Interview and academic factors. The exact weighting varies by
          cycle — the components below are structural; verify the current
          weights on the official source.
        </>
      }
      ...
      schema={[
        articleSchema({
          headline: `${institute.name}: Composite Score Explained`,
      ...
      <Section id="components" heading="Composite components">
        <p>
          <strong className="text-ink">
            Structure (weights to be verified):
          </strong>{" "}
          {composite.notes}
        </p>
        <ComparisonTable
          ...
          rows={composite.components.map((c) => ({
            component: c.label,
            weight:
              c.weight == null ? <Tbd label="weight %" /> : `${c.weight}%`,
          }))}
        />
        <p className="text-sm">
          See overall{" "}
          ...
          , aim for a{" "}
          <a href="/ipmat/safe-score" className="text-brand hover:underline">
            safe score
          </a>
          , or start{" "}
