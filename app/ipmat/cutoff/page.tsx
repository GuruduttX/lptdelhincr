...
export default function CutoffHub() {
  return (
    <HubPage
      ...
      intro={
        <>
          IPMAT selection combines a <strong>sectional/overall cutoff</strong>{" "}
          (you may need to clear each section) with a{" "}
          <strong>composite score</strong> — your aptitude result plus the
          Personal Interview and academic factors. Cutoffs move yearly, so treat
          any figure as a guide and verify against the official institute
          source.
        </>
      }
      ...
      faqs={ipmatFaqs
        .filter((f) => f.cluster === "selection")
        .map((f) => ({ q: f.q, a: f.a }))}
      schema={[
        articleSchema({
          headline: "IPMAT Cutoffs & Composite Score",
    ...
    >
      <div className="space-y-10">
        <Section id="cutoffs" heading="IPMAT sectional cutoffs">
          <p>
            <strong className="text-ink">
              Sectional cutoffs are cleared independently
            </strong>{" "}
            where an institute sets them — missing one can disqualify a strong
            overall score. Verified figures will be filled in here; placeholders
            are clearly marked.
          </p>
          <ComparisonTable
        ...
        <Section id="composite" heading="How the composite score works">
          <p>
            <strong className="text-ink">
              Aptitude alone doesn't decide the offer.
            </strong>{" "}
            Each institute combines your IPMAT aptitude score with the Personal
            Interview and academic/diversity factors into a composite (exact
            weights vary by cycle). See the per-institute breakdowns and
            safe-score guidance in the cards below.
          </p>
        </Section>
      </div>
    </HubPage>
