...
export const metadata: Metadata = pageHead({
  ...
  description:
    "Free CUET & IPMAT resources — college predictor, cutoff dataset, syllabus and marking guides.",
  path: "/resources",
});
...
export default function Resources() {
  return (
    <HubPage
      ...
      intro={
        <>
          Free tools and reference data for CUET and IPMAT aspirants — the
          college predictor, the cutoff dataset, syllabus and the IPMAT marking
          guide. More datasets roll out in phases.
        </>
      }
      groups={[
        { heading: "Free tools & guides", icon: "FileText", items: ready },
      ]}
      ctaMessage="Hi LPT Delhi-NCR, I need help with CUET/IPMAT resources"
      schema={[
        itemListSchema(
          ready.map((r) => ({ name: r.label, url: `${DOMAIN}${r.href}` })),
        ),
      ]}
    >
      <Section id="coming" heading="Coming soon">
        ...
        <ul className="list-disc space-y-1 pl-5">
          {planned.map((p) => (
            <li key={p} className="text-body">
              {p}{" "}
              <span className="ml-1 rounded bg-cream-soft px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-body/70">
                Soon
              </span>
            </li>
          ))}
