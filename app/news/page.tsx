...
const faqs: FAQItem[] = [
  {
    q: "Where can I find the latest CUET & IPMAT updates?",
    a: (
      <>
        Time-sensitive, sourced updates appear here and on each vertical's
        changes page, while evergreen guides live in our journal. We date every
        update and verify it against the official source before publishing —
        never rumours.
      </>
    ),
  },
...
export default function News() {
  ...
  return (
    <ContentPage
      ...
      intro={
        <>
          Time-sensitive CUET and IPMAT updates, dated and sourced. We publish a
          news item only after verifying it against the official source. For
          evergreen strategy, see the{" "}
        ...
        {items.length === 0 ? (
          <p>
            <strong className="text-ink">
              Verified, dated updates appear here
            </strong>{" "}
            as official notices land: <Tbd label="news items" />. We never post
            unverified rumours. Meanwhile, track{" "}
          ...
          <ul className="space-y-3">
            {items.map((p) => (
              <li key={p.slug}>
                <a
                  href={`/${p.vertical.toLowerCase()}/news/${p.slug}`}
