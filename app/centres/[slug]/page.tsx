...
import { centreParams } from "@/lib/static-params";
...
export default async function CentrePage({
  ...
  const { centre } = load(await params);
  ...
  const faqs: FAQItem[] = [
    {
      ...
      a: (
        <>
          {centre.fullAddress}, near {centre.landmark}. The centre offers CUET
          and IPMAT coaching in offline, online and hybrid modes.
        </>
      ),
    },
    {
      ...
      a: (
        <>
          CUET (UG) and IPMAT/JIPMAT, across offline, online and hybrid batches.
        </>
      ),
    },
    {
      ...
      a: (
        <>
          Yes — book a free demo on WhatsApp and our team will confirm the next
          batch and timings for the {centre.area} centre.
        </>
      ),
    },
  ];
  ...
  return (
    <ContentPage
      ...
      intro={
        <>
          {BRAND_SHORT}'s {centre.area} centre offers CUET and IPMAT coaching at{" "}
          {centre.fullAddress}. Landmark: {centre.landmark}. Offline, online and
    ...
    >
      <Section id="address" heading="Address & directions">
        <p className="flex items-start gap-2">
          ...
          <span>
            ...
            <br />
            <strong className="text-ink">Nearest landmark / metro:</strong>{" "}
            {centre.landmark}
          </span>
        </p>
        <div className="flex flex-wrap gap-3 pt-2">
          ...
          <a
            href={whatsappLink(
              `Hi ${BRAND_SHORT}, directions to the ${centre.area} centre`,
            )}
            target="_blank"
            rel="nofollow"
            className="btn-primary"
          >
            <MessageCircle className="h-4 w-4" strokeWidth={2} /> Chat on
            WhatsApp
          </a>
          {phoneKnown && (
        ...
        <p className="mt-6 text-sm">
          <strong className="text-ink">Areas served:</strong>{" "}
          {centre.areaServed.join(", ")}.
        </p>
      ...
      <Section id="batches" heading={`CUET & IPMAT batches at ${centre.area}`}>
        <p>
          This centre runs CUET and IPMAT batches in offline, online and hybrid
          modes. See full course details and book a demo:
        </p>
        <div className="flex flex-wrap gap-3">
          <Link href="/courses/cuet" className="btn-secondary">
            CUET courses
          </Link>
          <Link href="/courses/ipmat" className="btn-secondary">
            IPMAT courses
          </Link>
        </div>
        <p className="text-sm">
          Local pages:{" "}
          <a
            href={`/cuet/coaching-in-${centre.slug}`}
          ...
          ·{" "}
          <a
            href={`/ipmat/coaching-in-${centre.slug}`}
            className="text-brand hover:underline"
          >
      ...
      <Section
        id="why"
        heading={`Why students choose the ${centre.area} centre`}
      >
        <p>
          Accessible from {centre.areaServed.slice(1).join(", ")} and around{" "}
          {centre.landmark}. Faculty, results and local differentiators specific
          to this centre will be added from verified records.
        </p>
        <p className="text-sm">
