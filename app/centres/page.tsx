...
export const metadata: Metadata = pageHead({
  ...
  description:
    "LPT Delhi-NCR centres for CUET & IPMAT coaching: Noida, Hauz Khas, GTB Nagar and Gurugram. Addresses, directions and batches.",
  path: "/centres",
});
...
export default function CentresIndex() {
  return (
    <HubPage
      ...
      intro={
        <>
          {BRAND} runs {CENTRES.length} centres across Delhi-NCR — Noida (Sector
          62), Hauz Khas, GTB Nagar and Gurugram (Sector 14) — offering CUET and
          IPMAT coaching in offline, online and hybrid modes. Find your nearest
          centre below.
        </>
      }
      groups={[
      ...
      schema={[
        itemListSchema(
          CENTRES.map((c) => ({ name: c.name, url: `/centres/${c.slug}` })),
        ),
