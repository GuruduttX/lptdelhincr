...
import { facultyParams } from "@/lib/static-params";
...
export default async function FacultyBio({
  params,
}: {
  params: Promise<Params>;
}) {
  ...
  return (
    <ContentPage
      ...
      intro={
        <>
          {f.name} is a {f.role.toLowerCase()} at {BRAND}, specialising in{" "}
          {f.subject} for CUET and IPMAT aspirants, with {f.experienceYears}{" "}
    ...
    >
      <Section id="bio" heading="About">
        <p>
          {f.name} teaches {f.subject} across {BRAND}'s Delhi-NCR centres, with{" "}
          {f.experienceYears} years of experience preparing students for CUET
          and IPMAT. The focus is on building concepts from the basics and
          drilling exam-day technique through structured practice and mentoring.
        </p>
      </Section>
