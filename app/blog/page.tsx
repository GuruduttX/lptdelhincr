...
export default function BlogIndex() {
  ...
  return (
    <ContentPage
      ...
      intro={
        <>
          Strategy guides and decoded patterns for CUET and IPMAT, written by
          the people who teach. Every post is dated and sourced; we explain
          process and method, never inflated stats.
        </>
      }
    ...
    >
      <Section id="posts" heading="Latest posts">
        <div className="grid gap-4 sm:grid-cols-2">
          {items.map((p) => (
            <a
              key={p.slug}
              href={`/blog/${p.slug}`}
              className="lift-card block rounded-2xl border border-hairline bg-white p-5"
            >
              <div className="text-[11px] font-semibold uppercase tracking-wide text-body">
                {p.vertical} · {p.dateDisplay}
              </div>
        ...
        <p className="text-sm">
          Looking for time-sensitive updates? See{" "}
