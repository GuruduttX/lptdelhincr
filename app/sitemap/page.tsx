  ...
  return (
    <article className="container-lpt py-10 md:py-14">
      ...
      <header className="mt-6 max-w-3xl">
        <h1 className="h-display text-3xl sm:text-4xl md:text-[2.6rem] leading-tight">
          Sitemap
        </h1>
        <p className="answer-lead mt-5 border-l-[3px] border-brand/60 pl-4 text-body leading-relaxed">
          Every page on {BRAND} in one place — browse the full CUET and IPMAT
          guides, tools, cutoffs, syllabus, coaching and centre pages, grouped
          by section.
        </p>
      </header>
            ...
            {getVerticalTree(v).map((cl) => (
              ...
              >
                ...
                <ul className="mt-3 space-y-1.5 text-sm">
                  {cl.entries.map((e) => (
                    <li key={e.href}>
                      <a href={e.href} className="text-body hover:text-brand">
                        {e.label}
                      </a>
                    </li>
                  ))}
      ...
      <section className="mt-12" aria-label="Centres sitemap">
        ...
        <ul className="mt-5 flex flex-wrap gap-2">
          <li>
            <a href="/centres" className="btn-pill">
              All centres
            </a>
          </li>
          {CENTRES.map((c) => (
            <li key={c.slug}>
              <a href={`/centres/${c.slug}`} className="btn-pill">
                {c.area}
              </a>
            </li>
          ))}
      ...
      <section className="mt-12" aria-label="Company sitemap">
        ...
        <ul className="mt-5 flex flex-wrap gap-2">
          {company.map((l) => (
            <li key={l.href}>
              <a href={l.href} className="btn-pill">
                {l.label}
              </a>
            </li>
          ))}
      ...
      <JsonLd
        schema={[
          breadcrumbSchema([{ name: "Home", item: "/" }, { name: "Sitemap" }]),
        ]}
      />
    </article>
  );
