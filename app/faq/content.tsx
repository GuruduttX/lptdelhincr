...
export default function MasterFaq() {
  ...
  return (
    <HubPage
      ...
      intro={
        <>
          A searchable knowledge base of {ALL.length} direct, sourced CUET and
          IPMAT answers, growing over time. Search below, or browse the
          clustered vertical FAQ hubs.
        </>
      }
      groups={[
        {
          ...
          items: [
            {
              label: "CUET FAQ hub",
              href: "/faq/cuet",
              desc: "Cutoffs, results, admission & more",
            },
            {
              label: "IPMAT FAQ hub",
              href: "/faq/ipmat",
              desc: "Exam, marking, selection & more",
            },
          ],
        },
      ]}
    ...
    >
      <div className="not-prose">
          ...
          <div className="flex gap-2">
            {(["all", "CUET", "IPMAT"] as const).map((v) => (
              <button
                ...
                onClick={() => setVert(v)}
                className={
                  vert === v ? "btn-primary px-4 py-2 text-sm" : "btn-pill"
                }
              >
                {v === "all" ? "All" : v}
              </button>
        ...
        <p className="mt-2 text-sm text-body" aria-live="polite">
          Showing {count} of {ALL.length} questions.
        </p>
          ...
          {visible.map(({ it, show }, i) => (
            <div key={i} className="py-4" hidden={!show}>
              <h3 className="text-base font-semibold text-ink">
                {it.q}{" "}
                <span className="ml-1 rounded bg-cream-soft px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-body">
                  {it.vertical}
                </span>
              </h3>
